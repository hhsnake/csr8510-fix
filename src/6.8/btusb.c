 ...
 * cmd_timeout callback above ever runs - so recovery must be driven from
 * close: if not a single HCI event arrived between open and close, reset.
 * Rate-limited module-wide because a reset rebinds the driver with fresh
 * flags, so per-device state cannot stop a dead dongle from looping.
 */
		       ...
		       const struct usb_device_id *id)
{
	 ...
	 * kernels (e.g. Ubuntu HWE 6.8) backported that removal. Only touch
